...
const displayCountries = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    card += `<a href="country.html?country=${
      arr[i].code
    }" class="text-decoration-none m-0 p-0">
                    <div class="country-card rounded pb-3">
                        <div class="flag ratio ratio-16x9">
                            <img src="${arr[i].flag}" alt="Flag of ${
      arr[i].name
    }" class="img-fluid">
                        </div>
                        <div class="card-body d-flex flex-column py-4 px-4">
                            <h2 class="country-name h5 fw-800 mb-3">${
                              arr[i].name
                            }</h2>
                            <p class="fw-600 fs-small mb-1">Population: <span class="population fw-300 opacity-9">${arr[
                              i
                            ].population.toLocaleString()}</span></p>
                            <p class="fw-600 fs-small mb-1">Continent: <span class="region fw-300 opacity-9">${
                              arr[i].continent
                            }</span></p>
...
const filterAfrica = () => {
  loadingScreen.classList.remove("hide");
  countries.innerHTML = "";
  let newCountriesArr = countriesArr.filter(
...
const filterAntarctica = () => {
  ...
  countries.innerHTML = "";
  let newCountriesArr = countriesArr.filter(
    (item) => item.continent == "Antarctica"
  );
  displayCountries(newCountriesArr);
};
